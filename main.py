...
def main():
    parser = argparse.ArgumentParser(
        description="Measuring Error in Business Analytics"
    )
    ...
    if args.data_path and args.data_path.exists():
        ...
        y_true = (
            100
            + 20 * np.sin(np.arange(config["data"]["n_samples"]) / 10)
            + np.random.normal(0, 5, config["data"]["n_samples"])
        )
        y_pred = y_true + np.random.normal(
            0,
            config["model"]["noise_level"] * y_true.std(),
